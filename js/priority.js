...
class PriorityEngine {
    ...
    _calculatePriorityScore(event) {
        ...
        if (event.type === 'known_person') score += 4; // 8 = High Priority Focus
    ...
    _classifyAlert(score, event) {
        // CRITICAL: Immediate danger (very close, fast approach, or high severity obstacle)
        if ((event.distance && event.distance < 1.0) ||
            (event.movement === 'approaching' && event.distance < 2.0) ||
            score >= 9) {
            return 'CRITICAL';
        }

        // HIGH: Needs attention but not instant danger
        if (score >= 6 || event.type === 'approach' || event.type === 'path_blocked' || event.severity === 'high' || event.class === 'person') {
            return 'HIGH';
        }

        // MEDIUM: Informational (helpful but not urgent)
        if (score >= 4 || event.severity === 'medium') {
            return 'MEDIUM';
        }
    ...
    _shouldFireAlert(event, classification, currentTime) {
        // CRITICAL events trigger INSTANTLY, no cooldown
        if (classification === 'CRITICAL') return true;

        // HIGH events (Danger/Collision) trigger unless recent
        if (classification === 'HIGH' && event.type !== 'known_person') {
            const lastTime = this.lastFiredTime.get(event._key);
            if (!lastTime) return true;
            return (currentTime - lastTime) > 3000;
        }

        // LOW priority events are ignored
        if (classification === 'LOW') return false;

        // MEDIUM priority (Awareness needed), UI only usually but we let it pass to VoiceEngine to handle
        const lastTime = this.lastFiredTime.get(event._key);
        if (!lastTime) return true;
