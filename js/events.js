...
class EventEngine {
    ...
    process(spatialObjects, frameW, frameH, emotions = [], behaviors = []) {
        ...
        this._detectKnownPersons(rawEvents, emotions);
    ...
    /** Known person detection — trigger an alert when a recognized face is in view */
    _detectKnownPersons(events, emotions) {
        emotions.forEach((e, idx) => {
            if (e.person && e.person !== "Unknown person" && e.person.toLowerCase() !== "unknown") {
                events.push({
                    type     : 'known_person',
                    label    : `${e.person} is right here`,
                    object   : 'person',
                    class    : 'person',
                    identity : { name: e.person },
                    position : e.direction || 'ahead',
                    distance : e.distance,
                    severity : 'medium', // High enough to trigger voice
                    _key     : `known_person_${e.person}`
                });
            }
        });
    }
    ...
    _validateEvents(rawEvents) {
        ...
        rawEvents.forEach(evt => {
            ...
            if (this.pendingEvents.has(key)) {
                ...
                if (pending.count >= this.CONFIRM_FRAMES || evt.type === 'known_person') {
                ...
                // Immediately active for fast-reaction events like face recognition
                if (evt.type === 'known_person') {
                    this.activeEvents.push(evt);
                }
            }
        });
