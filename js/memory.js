...
class MemoryEngine {
    ...
    process(detections, events) {
        ...
        detections.forEach(obj => {
            if (obj.class === 'person') {
                // Priority: Use identity from backend if available (Netra Core AI)
                if (obj.person && obj.person !== 'Unknown' && obj.person !== 'Unknown person') {
                    const identity = {
                        id: obj.person_id || `core_${obj.person}`,
                        name: obj.person,
                        isKnown: true,
                        confidence: obj.person_confidence || obj.score,
                        lastSeen: currentTime
                    };
                    obj.identity = identity;
                    this.activeIdentities.set(obj._trackId, identity);
                    
                    if (!this.people[identity.id]) {
                        this.people[identity.id] = { ...identity, encounterCount: 1 };
                    } else {
                        this.people[identity.id].encounterCount++;
                        this.people[identity.id].lastSeen = currentTime;
                    }
