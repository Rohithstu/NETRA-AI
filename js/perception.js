...
class PerceptionEngine {
    ...
    connectToCore() {
        ...
        this.socket = new WebSocket(`${protocol}//${host}/ws/vision`);
        ...
        this.socket.onmessage = (event) => {
            ...
            // Proactive AI Speech
            if (data.proactive_audio && this.voice) {
                this.voice.speak(data.proactive_audio, true); // True = priority bypass
            }
    ...
    sendFrameToCore() {
        ...
        // Match backend's expected processing width but preserve aspect ratio
        ...
        octx.drawImage(this.video, 0, 0, targetWidth, targetHeight);

        const base64Img = offscreenCanvas.toDataURL('image/jpeg', 0.85); // Higher quality
    ...
    drawOverlays(objects, spatialResult, navState, emotions = [], behaviors = []) {
        ...
        objects.forEach(obj => {
            ...
            // Danger alert shake trigger (Layer 4 integration)
            // if (obj.risk === 'immediate' && !this.canvas.parentElement.classList.contains('danger-alert')) {
            //     this.canvas.parentElement.classList.add('danger-alert');
            //     setTimeout(() => this.canvas.parentElement.classList.remove('danger-alert'), 500);
            // }

            // Bounding box with dynamic pulsating glow
            this.ctx.lineWidth = obj.risk === 'immediate' ? 6 : 4;
            this.ctx.strokeStyle = colors.box;

            if (obj.risk === 'immediate' || (obj.class === 'person' && obj.identity?.isKnown)) {
                this.ctx.shadowColor = obj.identity?.isKnown ? '#22c55e' : colors.glow; // Green glow for known faces
                this.ctx.shadowBlur = obj.identity?.isKnown ? 25 : 20;
            ...
            // Draw box with slight corner rounding effect (by drawing rect manually)
            this.ctx.beginPath();
            this.ctx.roundRect ? this.ctx.roundRect(x, y, w, h, 8) : this.ctx.rect(x, y, w, h);
            this.ctx.stroke();
            ...
            const riskIcon = obj.risk === 'immediate' ? '🛑 ' : obj.risk === 'potential' ? '⚠️ ' : '';

            // Face Recognition Effect
            let isRecognizedFace = false;
            if (obj.class === 'person' && obj.identity) {
                label = obj.identity.isKnown ? `Recognized: ${obj.identity.name}` : 'Unknown Person';
                this.ctx.fillStyle = obj.identity.isKnown ? 'rgba(34, 197, 94, 0.9)' : 'rgba(59, 130, 246, 0.9)'; // Green for known, Blue for unknown
                isRecognizedFace = obj.identity.isKnown;
            } else {
                this.ctx.fillStyle = colors.bg;
            }

            // High-visibility font with zoom effect for recognized faces
            this.ctx.font = isRecognizedFace ? 'bold 24px -apple-system, sans-serif' : 'bold 20px -apple-system, sans-serif';
            const text = `${riskIcon}${label.toUpperCase()} ${distLabel}`;
            const textW = this.ctx.measureText(text).width + 20;

            this.ctx.beginPath();
            this._roundRect(x, y - (isRecognizedFace ? 38 : 32), textW, (isRecognizedFace ? 36 : 30), 6);
            this.ctx.fill();
            ...
            if (obj.emotion) {
                ...
                const etw = this.ctx.measureText(emotionText).width + 14;
                this.ctx.fillStyle = 'rgba(124, 58, 237, 0.95)';
                this._roundRect(x, y + 2, etw, 24, 4);
                this.ctx.fill();
                this.ctx.fillStyle = '#FFFFFF';
                this.ctx.fillText(emotionText, x + 7, y + 20);
            }
