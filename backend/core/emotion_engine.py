...
try:
    ...
    try:
        from fer.fer import FER  # type: ignore  # Fallback for newer versions like 25.x+
    except (ImportError, ModuleNotFoundError):
        print("❌ CRITICAL: 'fer' library not found. Please run: pip install fer")
...
class EmotionEngine:
    ...
    def __init__(self, use_depth=True, use_face_recognition=True):
        ...
        if FER is not None:
            try:
                ...
                print(f"❌ Failed to initialize FER classifier: {e}")
                self.emotion_detector = None
        else:
            print("❌ Skill: FER library is missing. Emotion detection engine is disabled.")
            self.emotion_detector = None
    ...
    def process_frame(self, frame):
        ...
        if self.face_engine:
            _, _fd = self.face_engine.process_frame(frame)  # type: ignore
            face_detections = list(_fd)
        
        detections = []
        fer_matched_indices = set()
        ...
        for result in emotion_results:
            ...
            # Try to match with known face identity using Euclidean distance
            ...
            for f_idx, fd in enumerate(face_detections):
                bx = (fd["bounding_box"][0] + fd["bounding_box"][2]) // 2
                by = (fd["bounding_box"][1] + fd["bounding_box"][3]) // 2
                dist = ((bx - center_x) ** 2 + (by - center_y) ** 2) ** 0.5
                if dist < 150 and dist < best_dist:  # 150 pixels tolerance
                    best_dist = dist
                    best_idx = f_idx
            
            if best_idx != -1:
                person = face_detections[best_idx].get("person", "Unknown")  # type: ignore
                person_name = person if person != "Unknown" else "Unknown person"
                person_confidence = face_detections[best_idx].get("confidence", 0.0)
                fer_matched_indices.add(best_idx)
            
            # Priority filter
            reportable = self.should_report(intensity, distance)
            if person_name != "Unknown person" and person_name.lower() != "unknown":
                reportable = True  # Always report recognized people!
        ...
        # Add any FaceEngine detections that FER missed entirely
        for f_idx, fd in enumerate(face_detections):
            if f_idx in fer_matched_indices:
                continue
            person = fd.get("person", "Unknown")
            if person == "Unknown" or person.lower() == "unknown":
                continue  # Skip unknown unmatched faces
            
            zone = fd.get("direction", "center")
            dist = fd.get("distance", None)
            
            detection = {
                "person": person,
                "person_confidence": round(float(fd.get("confidence", 0.8)), 2),
                "emotion": "neutral",
                "intensity": "medium", # Promote to medium so it gets reported
                "confidence": 1.0,
                "direction": zone,
                "reportable": True,
                "bounding_box": fd.get("bounding_box"),
            ...
            print(f"  {person} found actively (FER bypassed) [{zone}]")
