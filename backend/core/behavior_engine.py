...
# MediaPipe availability and import handling
try:
    import mediapipe as mp  # type: ignore
    # Attempt to import submodules early to verify installation completeness
    import mediapipe.tasks.python as mp_tasks_python  # type: ignore
    from mediapipe.tasks.python import vision  # type: ignore
    MEDIAPIPE_AVAILABLE = True
except (ImportError, BaseException) as e:
    # Some older or incomplete installations might fail here
    mp = None  # type: ignore
...
class BehaviorEngine:
    ...
    def __init__(self, use_depth=True):
        ...
        if not MEDIAPIPE_AVAILABLE:
            print("⚠️ MediaPipe not available or failed to import.")
            print("💡 To fix: py -m pip install mediapipe")
            if 'sys.version_info' in globals() and sys.version_info >= (3, 13):
                 print("⚠️ Note: MediaPipe has known stability issues on Python 3.13.4.")
            self.detector = None
            self.mp_pose = None
            self.mp_draw = None
        else:
            try:
                # Use explicit submodule access to satisfy editor's static analysis (Pyright/Pylance)
                import mediapipe.tasks.python as python  # type: ignore
                from mediapipe.tasks.python import vision  # type: ignore
                
                # Initialize these so process_frame doesn't crash
                self.mp_pose = mp.solutions.pose  # type: ignore
                self.mp_draw = mp.solutions.drawing_utils  # type: ignore
                
                model_path = os.path.join(os.path.dirname(__file__), "pose_landmarker_lite.task")
                if not os.path.exists(model_path):
                    print(f"⚠️ Pose model not found at {model_path}. Pose detection disabled.")
                    self.detector = None
                else:
                ...
                print(f"❌ Failed to load PoseLandmarker (Python 3.13 mediapipe incompatibility): {e}")
                self.detector = None
                self.mp_pose = None
                self.mp_draw = None
    ...
    def detect_movement(self, current_landmarks, h, w):
        ...
        """
        if not current_landmarks or not self.prev_landmarks:
    ...
    def process_frame(self, frame):
        ...
        from mediapipe.solutions.drawing_utils import DrawingSpec  # type: ignore
